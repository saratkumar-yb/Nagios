...
# Just in case of problyms, let's not hang Nagios
$SIG{'ALRM'} = sub {
...
unless ($dbh = DBI->connect("DBI:mysql:database=$database;host=$server","$username","$password",{RaiseError => 1, PrintError => 0})) {
	printf "SQL CRITICAL: Can't connect to mysql server $DBI::errstr\n";
...
sub syntax {
	...
	printf("Error: ($s)\n") unless ($help);
	printf("Runs a query against a MY-SQL server or Mariadb server and returns the first row\n");
